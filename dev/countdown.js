...
// чтобы полоса появилась сразу и страница не сдвигалась при загрузке
{
  ...
  if (deadline > Date.now()) {
    ...
    // Подпись и цифры — в блоках фиксированной ширины (см. style.css), стрелка ↗ — рядом с цифрами
    const timer = document.createElement('span');
    timer.className = 'event-bar-timer';
    const value = document.createElement('b');
    value.setAttribute('role', 'timer');
    const digits = document.createElement('span');
    digits.className = 'event-bar-digits';
    value.append(digits);
    const label = document.createElement('span');
    label.className = 'event-bar-label';
    label.textContent = 'До конца регистрации';
    ...
    const tick = () => {
      ...
      digits.textContent = `${pad(days)} дн ${pad(hours)}:${pad(minutes)}:${pad(seconds)}`;

      // Следующее обновление — в начале следующей секунды, чтобы отсчёт шёл ровно
      tickTimeout = setTimeout(tick, 1000 - (Date.now() % 1000) + 20);
    };

    // В фоновой вкладке браузер замораживает таймеры — при возвращении сразу показываем точное время
